...
import {
  ...
  amritaSiddhiYoga,
  bhadraWindows,
  muhurtaWindows as _muhurtaWindows,
...
describe("abhijitMuhurta", () => {
  ...
  it("is the 8th day muhurta (midday)", () => {
    ...
    const expectedStart = new Date(wins[8][0] * 86400000 - 2440587.5 * 86400000)
      .toISOString()
      .slice(0, 19);
...
describe("varjyamWindow / amritKalamWindow", () => {
  ...
  it("varjyam and amrit windows have positive duration", () => {
    ...
    expect(new Date(v.end).getTime()).toBeGreaterThan(
      new Date(v.start).getTime(),
    );
    expect(new Date(a.end).getTime()).toBeGreaterThan(
  ...
  it("amrit window starts ~26.67 ghatikas after varjyam window", () => {
    ...
    const actualDiff =
      (((aStartMs - vStartMs) % nakDurMs) + nakDurMs) % nakDurMs;
    ...
    const karanas = [
      {
        index: 6,
        name: "Vishti",
        starts_at: "2024-01-01T06:00:00Z",
      ...
      },
      {
        ...
        ends_at: "2024-01-01T18:00:00Z",
      },
      {
        index: 6,
        name: "Vishti",
        starts_at: "2024-01-01T18:00:00Z",
...
describe("computeGowri", () => {
  ...
  it("Amridha, Sugam, Labam, Dhanam, Uthi are auspicious", () => {
    for (const seg of [...gowri.day, ...gowri.night]) {
      const expected = ["Amridha", "Sugam", "Labam", "Dhanam", "Uthi"].includes(
        seg.name,
      );
  ...
  it("Monday (1) day starts with Amridha (auspicious)", () => {
    const mondayGowri = computeGowri(
      sunriseJd,
      sunsetJd,
      nextSunriseJd,
      1,
...
describe("computeNallaNeram", () => {
  ...
  it("subtracts inauspicious window from auspicious hora", () => {
    ...
    const bad = [
      { start: "2024-01-01T08:30:00Z", end: "2024-01-01T09:00:00Z" },
    ];
  ...
  it("returns empty when inauspicious covers entire auspicious period", () => {
    ...
    const bad = [
      { start: "2024-01-01T07:00:00Z", end: "2024-01-01T10:00:00Z" },
    ];
    const result = computeNallaNeram(horas, bad);
